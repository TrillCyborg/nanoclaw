 ...
 */
export async function handleTelegramCommandIpc(
  data: any,
  ...
  const writeResult = (result: { success: boolean; message: string; data?: any }) => {
  ...
  try {
    switch (type) {
      ...
      case 'telegram_set_commands': {
        ...
        const commandList = commands.map((c: TelegramCommand) => `/${c.command}`).join(', ');
        writeResult({
          success: true,
      ...
      case 'telegram_add_command': {
        ...
        const existingCommands: TelegramCommand[] = getResult.result || [];
        ...
        const setResult = await setResponse.json() as ApiResponse;
